pageextension 50038 PostedSalesInvoicesList extends "Posted Sales Invoices"
{
    layout
    {
    ...
    actions
    {
        addlast(processing)
        {
            action("Sales Tax Invoice")
            {
                ...
                trigger OnAction()
                var
                    shimpmentheader: Record "Sales Invoice Header";

                begin
                    shimpmentheader.Reset();
                    shimpmentheader.SetRange("No.", Rec."No.");
                    Report.RunModal(50143, true, true, shimpmentheader);
